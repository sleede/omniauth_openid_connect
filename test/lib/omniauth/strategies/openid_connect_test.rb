  ...
  module Strategies
    class OpenIDConnectTest < StrategyTestCase # rubocop:disable Metrics/ClassLength
      ...
      def test_callback_phase_with_jwks_uri
        ...
        stub_request(:get, strategy.options.client_options.jwks_uri)
          .to_return(
            status: 200,
      ...
      def test_option_client_auth_method
        ...
        stub_request(:post, "#{ opts.scheme }://#{ opts.host }:#{ opts.port }#{ opts.token_endpoint }")
          .with(body: { scope: 'openid', grant_type: 'client_credentials', client_id: @identifier, client_secret: @secret })
          .to_return(
            status: 200,
            headers: { content_type: 'application/json' },
            body: json_response
